...
- **Status endpoint** — `statusUrl` records where a consumer can ask the service *itself* how it is doing, when the service publishes such an endpoint. It is a pointer, not a measurement: the registry neither polls it nor derives `status` from it. Set on the entries that were confirmed to serve one and that it is appropriate to advertise (9 as of 2026-09-09). Absence means this registry does not point at one: the service publishes none, or it publishes one this registry chooses not to advertise — `algora` is the latter, an archived service whose health endpoint still reports live budget and scheduler detail. Absence never means the service is down.
